...
<div class="row">
    <div class="col-lg-9 col-xl-8">
        <form method="post" action="<?= e($action) ?>" novalidate data-no-double-submit enctype="multipart/form-data">
            <?= csrf_field() ?>
            ...
            <div class="lrms-card">
                <div class="lrms-card-header">
                    <h5>Location & Photo</h5>
                </div>
                <div class="lrms-card-body">
                    <div class="row g-3">
                        <div class="col-md-6">
                            <label class="form-label" for="latitude">Latitude</label>
                            <input type="text" class="form-control<?= has_error($errors, 'latitude') ?>"
                                   id="latitude" name="latitude"
                                   value="<?= $value('latitude') ?>" readonly
                                   placeholder="Auto-captured via GPS">
                            <div class="form-text">Auto-captured from device GPS location.</div>
                            <?= field_error($errors, 'latitude') ?>
                        </div>

                        <div class="col-md-6">
                            <label class="form-label" for="longitude">Longitude</label>
                            <input type="text" class="form-control<?= has_error($errors, 'longitude') ?>"
                                   id="longitude" name="longitude"
                                   value="<?= $value('longitude') ?>" readonly
                                   placeholder="Auto-captured via GPS">
                            <div class="form-text">Auto-captured from device GPS location.</div>
                            <?= field_error($errors, 'longitude') ?>
                        </div>

                        <div class="col-12">
                            <button type="button" class="btn btn-sm btn-outline-primary" id="capture-location-btn">
                        ...
                        <div class="col-12">
                            <label class="form-label" for="photo">Visit Photo (Optional)</label>
                            <input type="file" class="form-control<?= has_error($errors, 'photo') ?>"
                                   id="photo" name="photo" accept="image/*" 
                                   capture="environment">
                            <div class="form-text">Upload a photo from your device (camera or gallery). Optional.</div>
                            <?php if ($isEdit && $value('photo_path')): ?>
                                <div class="mt-2">
                                    <img src="<?= e(url('/uploads/bc-visits/' . basename($value('photo_path')))) ?>" 
                                         alt="Visit photo" class="img-thumbnail" style="max-width: 200px; max-height: 200px;">
                                </div>
                            <?php endif; ?>
...
<!-- AJAX Script for auto-populating agent details and capturing location -->
<?php if (!$isEdit): ?>
<script>
// ========================================================================
// Location Capture
// ========================================================================
document.getElementById('capture-location-btn').addEventListener('click', function(e) {
    e.preventDefault();
    const statusEl = document.getElementById('location-status');
    
    if (!navigator.geolocation) {
        statusEl.innerHTML = '<span class="text-danger"><i class="fas fa-exclamation-circle"></i> Geolocation not supported</span>';
        return;
    }

    statusEl.innerHTML = '<span class="text-info"><i class="fas fa-spinner fa-spin"></i> Capturing location...</span>';
    ...
    navigator.geolocation.getCurrentPosition(
        function(position) {
            ...
            statusEl.innerHTML = '<span class="text-success"><i class="fas fa-check-circle"></i> Location captured</span>';
        },
        function(error) {
            ...
            statusEl.innerHTML = '<span class="text-danger"><i class="fas fa-times-circle"></i> ' + message + '</span>';
        },
        {
...
// ========================================================================
// Agent Details Auto-population via AJAX
// ========================================================================
