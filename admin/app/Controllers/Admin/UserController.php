...
final class UserController extends Controller
{
    ...
    public function create(Request $request): void
    {
        ...
        // BCBF Code is assigned by the system when a staff member is added, not typed
        // in on the form any more - see User::generateBcbfCode(). Overwrites whatever
        // bcExtraDetails() read from the (now removed) form field, so an old or
        // scripted submission still cannot set its own code.
        $data['bcbf_code'] = User::generateBcbfCode();
        ...
        Session::flash('success', sprintf(
            'User <strong>%s</strong> created. Employee code <code>%s</code>, BCBF Code <code>%s</code>, '
            . 'temporary password <code>%s</code>. They will be asked to change it at first sign-in.',
            e($data['name']),
    ...
    public function edit(Request $request): void
    {
        ...
        // BCBF Code is system-generated once, at creation, and is no longer a form
        // field - see User::generateBcbfCode(). Carry the existing value forward on
        // every edit so an edit submission (which no longer posts this field) cannot
        // blank it out. A user with no code yet (created before this change) is
        // assigned one now, rather than being left permanently without one.
        $data['bcbf_code'] = $user['bcbf_code'] ?? User::generateBcbfCode();
    ...
    private function validate(Request $request, ?int $ignoreId): Validator
    {
        ...
        $rules = [
            ...
            // BCBF Code has no validation rule: it is not a form input any more, it is
            // generated by User::generateBcbfCode() when the account is created.
    ...
    /**
     * BC Basic Details and Address Details: everything about a BC Supervisor's registered
    ...
    private function bcExtraDetails(Request $request): array
    {
        return [
            ...
            // BCBF Code is NOT read from the request any more - it is system-generated.
            // See UserController::create()/edit(), which set 'bcbf_code' on $data
            // themselves, after this array has been merged in.
    ...
    private function assertManageable(array $user): void
    {
        if (!Auth::isSuperAdmin() && (string) ($user['role_slug'] ?? '') === 'super_admin') {
            $this->back('/users', 'danger', 'You cannot manage a Super Supervisor account.');
        }
